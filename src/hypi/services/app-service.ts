...
export default class AppService {
  ...
  async findAppByName(name: string) {
    ...
    const findAppRes = await this.findApp(arcql)
    return findAppRes
  }
  ...
  async createUserApp(appDoc: any) {
    ...
    const updatedAppDoc = await this.replaceAppTypesWithSchematext(appDoc)
    appDoc = updatedAppDoc
    const values = {
      values: {
        App: appDoc,
      },
  ...
  async createAppFile(data: any) {
    ...
    const createAppRes = await Utils.writeToFile(this.getFilePath(), String(doc))
    return createAppRes
  }
